...
class Vehicle:
    ...
    def blocked_by_lead_vehicle(self, vehicles, controller):
        # Emergency vehicles get a protected corridor during EMERGENCY mode.
        # They should not queue behind normal vehicles.
        ...
        for other in vehicles:
            ...
            # Cars that are already well beyond the intersection should not
            # hold vehicles behind them in place.
    ...
    def can_move(self, controller, vehicles):
        # Emergency vehicle:
        # Before EMERGENCY phase it may approach the stop line,
        # but must not enter until priority is granted.
        if self.emergency:
            if controller.phase != Phase.EMERGENCY and self.is_approaching_stop_line():
                front = self.first_position()
                stop_pos = self.stop_line_position()
                if self.direction in (Direction.n_s, Direction.w_e):
                    if front + self.speed * 0.02 >= stop_pos:
                        return False
                else:
                    if front - self.speed * 0.02 <= stop_pos:
                        return False
                return not self.blocked_by_lead_vehicle(vehicles, controller)
            return True

        # Normal vehicles during emergency:
        # stop before entering, but let already-committed cars clear out.
        if controller.phase == Phase.EMERGENCY:
            if self.is_approaching_stop_line():
                front = self.first_position()
                stop_pos = self.stop_line_position()
                if self.direction in (Direction.n_s, Direction.w_e):
                    if front + self.speed * 0.02 >= stop_pos:
        ...
        # Normal vehicles during pedestrian WALK:
        # same rule: no new entry, but vehicles already past the line keep moving.
        ...
        # On red/yellow, stop only while still approaching the stop line.
        if not allow and self.is_approaching_stop_line():
            front = self.first_position()
            stop_pos = self.stop_line_position()
            if self.direction in (Direction.n_s, Direction.w_e):
                if front + self.speed * 0.02 >= stop_pos:
...
class TrafficController:
    ...
    def update(self, dt, vehicles):
        ...
        if self.phase == Phase.green_n_s:
            ...
            self.set_all_red()
            wait_time = EMERGENCY_CLEARANCE if self.all_red_reason == "emergency" else timer_all_red
            if self.phase_timer >= wait_time:
                if self.all_red_reason == "emergency" and self.emergency_request:
                    self.start_emergency_phase()
                elif self.ped_request:
                    self.start_ped_walk()
                elif self.all_red_reason == "to_ew":
            ...
            for v in vehicles:
                if v.emergency and v.direction == self.emergency_direction and not v.removed:
    ...
    def draw_status(self, surface):
        ...
        draw_text(
            surface,
            "Controls: P = pedestrian request | E = emergency | R = normal vehicle",
            SMALL_FONT,
            black,
            (20, 95),
        )
...
def main():
    ...
    rtos.add_task(Task("background", 0.10, 3, lambda: None))
